...
export function createSearchResultsListReducer(searchResultFor) {
    return (state, action) => {
        const { name } = action;
        const isInitializationCall = state === undefined;
        if(name !== searchResultFor && !isInitializationCall) return state;

        return searchResultsList(state, action);
    };
}
